...
var conn = null;
function websocket(clientcode, feedtoken, script1, task1) {
    var x1 = 0;
    var feed_token = feedtoken;
    var client_code = clientcode;
    var strwatchlistscrips1 = script1; //"nse_cm|2885";
    var task1 = task1; //'mw'; 
    var conn1 = new WebSocket("wss://wsfeeds.angelbroking.com/NestHtml5Mobile/socket/stream"); //('wss://omnefeeds.angelbroking.com/NestHtml5Mobile/socket/stream');
    this.connection = function() {
        return new Promise((resolve, reject)=>{
            ...
            conn1.onerror = function(evt) {
                console.log("error::", evt);
            };
            conn1.onclose = function(evt) {
...
function _atos(array) {
    var newarray = [];
    try {
        for(var i = 0; i < array.length; i++)newarray.push(String.fromCharCode(array[i]));
    } catch (e) {}
    return newarray.join("");
}
function trigger(e, args) {
    if (!triggers[e]) return;
    for(var n = 0; n < triggers[e].length; n++)triggers[e][n].apply(triggers[e][n], args ? args : []);
}

//# sourceMappingURL=watch_list.0ad1a3c3.js.map
